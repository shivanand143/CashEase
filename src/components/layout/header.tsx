...
export default function Header() {
  ...
  const pathname = usePathname(); // Get current path
   ...
   const router = React.useRef<HTMLAnchorElement>(null); // Using ref for router logic
  ...
  const handleSearchSubmit = (event: React.FormEvent) => {
      ...
      // router.current?.click(); // This doesn't work directly, need a different approach
      // Using window.location for simplicity, consider Next's router for better SPA behavior
        ...
        {/* Search Bar - More prominent */}
         <div className="flex-1 hidden md:flex justify-center px-4">
             <form onSubmit={handleSearchSubmit} className="w-full max-w-md relative">
                 ...
                 />
                  {/* Hidden link for navigation logic */}
                 {/* <Link ref={router} href={`/search?q=${encodeURIComponent(searchTerm.trim())}`} className="hidden">Search</Link> */}
             </form>
         </div>
