...
import ProductCard from '@/components/product-card'; // Ensure this is a default import if ProductCard is default exported
...
export default function HomePage() {
  ...
  React.useEffect(() => {
    ...
    const loadAllData = async () => {
      ...
      // Fetch Today's Picks Products (Generic)
      setLoadingTodaysPicks(true);
      try {
        console.log("HomePage: Fetching today's picks products (generic)...");
      ...
      if (errors.length > 0 && isMounted) {
        const errorMessage = `Failed to load: ${errors.join(', ')}. Some content may be missing. Please try refreshing.`;
