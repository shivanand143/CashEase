...
import { Card, CardContent, CardFooter } from '@/components/ui/card'; // Adjusted imports
import { Skeleton } from '@/components/ui/skeleton';
import { AlertCircle, ArrowLeft, ShoppingBag, Tag } from 'lucide-react';
...
// Combine Coupon with basic Store info for display within category
...
export default function CategoryPage() {
  ...
  const [coupons, setCoupons] = useState<CouponWithStore[]>([]); // State for category-specific coupons
  const [loadingStores, setLoadingStores] = useState(true);
  const [loadingCoupons, setLoadingCoupons] = useState(true); // Loading state for coupons
  const [error, setError] = useState<string | null>(null);
  const [categoryName, setCategoryName] = useState<string>(''); // To display category name
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // 1. Fetch Stores in this category
        const storesCollection = collection(db, 'stores');
        const qStores = query(
            storesCollection,
            where('categories', 'array-contains', formattedCategoryName), // Query by category name (ensure case matches in Firestore)
            where('isActive', '==', true),
            orderBy('name', 'asc')
        );
        ...
        // 2. Fetch Coupons for stores in this category
        if (storesData.length > 0) {
            const storeIds = storesData.map(s => s.id);
            const couponsCollection = collection(db, 'coupons');
             // Firestore 'in' query limit is 10, handle pagination or batching if needed for more stores
            ...
            // Sort combined coupons if needed (e.g., by featured status again)
             couponsData.sort((a, b) => Number(b.isFeatured) - Number(a.isFeatured));
             setCoupons(couponsData);

        } else {
             setCoupons([]); // No stores, so no coupons
        }
  ...
  const handleStoreClick = async (store: Store) => {
   ...
   const handleCouponClick = async (coupon: CouponWithStore) => {
       const targetUrl = coupon.link || '#'; // Fallback if coupon link missing
       if (user) {
           try {
